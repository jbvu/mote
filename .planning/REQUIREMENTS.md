...
- [x] **CLI-08**: On transcription failure, user is prompted to retry with kept WAV; orphaned WAVs detected on next `mote record` and offered for transcription
...
- [x] **INT-05**: User can upload transcripts to NotebookLM via `mote auth notebooklm` (experimental — unofficial API via notebooklm-py, sessions expire weekly)
